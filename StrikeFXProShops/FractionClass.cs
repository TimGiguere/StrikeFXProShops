		...
		public struct Fraction : IComparable, IFormattable
		{
			...
			public override string ToString()
			{
				...
				else
				{
                    ...
                    {
                        int WholeNumber = (int)(this.m_Numerator / this.m_Denominator);
                        int Remainder = (int)(this.m_Numerator % this.m_Denominator);
                        if (Remainder == 0)
                            return WholeNumber.ToString();
                        else
                        {
                            Fraction pFrac = new Fraction(Remainder, this.m_Denominator);
                            return String.Format("{0} {1}", WholeNumber, pFrac);
                        }
                    }
				}
			}
			...
			public static Fraction ToFraction(string inValue)
			{
				...
				else
				{
					...
					if (slashPos > -1)
					{
                        int spaceValue = inValue.IndexOf(' ');
                        long numerator = 0; 
                        long denominator = Convert.ToInt64(inValue.Substring(slashPos + 1));

                        if (spaceValue > 0)
                        {
                            long wholeNumber = Convert.ToInt64(inValue.Substring(0, spaceValue));
                            numerator = Convert.ToInt64(inValue.Substring(spaceValue + 1, slashPos - (spaceValue + 1)));
